...
h3(#customFcts). Custom preloaded functions
...
p. Each class referenced in configuration should feature a static method with the signature:

bc(sample). 
public static Collection<org.apache.cassandra.cql3.functions.Function> all()

p. This method will be called and the functions it returns will be made available to all keyspaces, just as are
